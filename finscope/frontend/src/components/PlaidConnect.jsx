...
export default function PlaidConnect() {
  ...
  const createLinkToken = useCallback(async () => {
    setError('')
    try {
      const { data } = await api.post('/api/plaid/create_link_token', { user_id: 'demo-user' })
  ...
  const onSuccess = useCallback(async (public_token, metadata) => {
    try {
      const { data } = await api.post('/api/plaid/exchange_public_token', { public_token })
  ...
  const fetchTxns = async () => {
    setError('')
    try {
      const { data } = await api.get('/api/plaid/transactions', { headers: { 'X-Plaid-Access-Token': accessToken } })
  ...
  return (
    <div className="rounded border border-slate-800 bg-slate-900 p-4">
      ...
      <div className="flex gap-2 mb-2">
        <button onClick={() => open()} disabled={!ready} className="px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 disabled:opacity-50">{accessToken ? 'Linked' : 'Connect Bank'}</button>
        <button onClick={fetchTxns} disabled={!accessToken} className="px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 disabled:opacity-50">Fetch Transactions</button>
